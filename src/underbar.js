(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection,function(last,current){
      if (!last){
      return false;
      }
  ...
  _.defaults = function(obj) {
    _.each(arguments,function(arg){
      _.each(arg,function(item,key){
        obj[key] = obj[key] === undefined ? item : obj[key];
      });
    });
  return obj;
  };
  ...
  _.shuffle = function(array) {
    ...
    for(var i = 1; i < result.length; ++i){
      result.splice(Math.floor(Math.random() * i),0,result.pop());
    }
    return result;
  };
  ...
  _.zip = function() {
    ...
    var longestLength = _.reduce(args,function(last,current){
  ...
  _.intersection = function() {
    ...
    var filterResult = function(arr){
      for(var i = result.length - 1; i >= 0; --i ){
        if(_.indexOf(arr,result[i]) === -1){
          result.splice(i,1);
        }
      }
    }//end of filterResult

    _.each([].slice.call(arguments,1),function(item){
      filterResult(item);
    });
